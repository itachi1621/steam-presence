...
def log(log_msg):
    print(f"[{datetime.now().strftime('%b %d %Y - %H:%M:%S')}] {log_msg}")

def error(error_msg):
    print(f"    ERROR: [{datetime.now().strftime('%b %d %Y - %H:%M:%S')}] {error_msg}")
...
def getGameDiscordID(loops=0):
    ...
    r = makeWebRequest("https://discord.com/api/v10/games/detectable")
...
def getImageFromDiscord():
    ...
    try:
        r = makeWebRequest(f"https://discord.com/api/v10/applications/{appID}/rpc")
